...
namespace CodeDeobfuscator
{
	public class HelperClass
	{
		...
		public static void SafeRemove(ILProcessor proc, Instruction target)
		{
			Mono.Cecil.Cil.MethodBody body = proc.Body;
			int targetOffset = target.Offset;
			foreach (ExceptionHandler handler in body.ExceptionHandlers)
			{
				//if ((targetOffset >= handler.HandlerStart.Offset) && (targetOffset < handler.HandlerEnd.Offset))
				if (handler.HandlerEnd.Previous == target)
				{
					handler.HandlerEnd = target.Previous;
				}
				else if (handler.HandlerEnd == target)
				{
					handler.HandlerEnd = target.Next;
				}
				if (handler.TryEnd.Previous == target)
				{
					handler.TryEnd = target.Previous;
				}
				else if (handler.TryEnd == target)
				{
					handler.TryEnd = target.Next;
				}
				if (handler.HandlerStart == target)
				{
					handler.HandlerStart = target.Next;
				}
				if (handler.TryStart == target)
				{
					handler.TryStart = target.Next;
				}
			}
			proc.Remove(target);
		}
		public static void SafeInsertBefore(ILProcessor proc, Instruction target, Instruction instruction)
		{
			Mono.Cecil.Cil.MethodBody body = proc.Body;
			int targetOffset = target.Offset;
			foreach (ExceptionHandler handler in body.ExceptionHandlers)
			...
			proc.InsertBefore(target, instruction);
		}
		public static void SafeInsertAfter(ILProcessor proc, Instruction target, Instruction instruction)
		{
			Mono.Cecil.Cil.MethodBody body = proc.Body;
			int targetOffset = target.Offset;
			foreach (ExceptionHandler handler in body.ExceptionHandlers)
		...
		public static void PatchInstructionReferences(Mono.Cecil.Cil.MethodBody body, Instruction _from, Instruction _to)
		{
			for (int _i = 0; _i < body.Instructions.Count; _i++)
			{
				Instruction curInstr = body.Instructions[_i];
				if (curInstr.Operand == _from)
				{
					curInstr.Operand = _to;
				}
				else if (curInstr.Operand is Instruction[])
				{
					Instruction[] instructionList = (Instruction[])curInstr.Operand;
					for (int __i = 0; __i < instructionList.Length; __i++)
					{
						if (instructionList[__i] == _from)
							instructionList[__i] = _to;
					}
				}
			}
		}
		...
		{
			T[] ret = findMembers<T>(module, type, false, comparers);
		...
		public static T[] findMembers<T>(ModuleDefinition module, object type, bool searchInNestedTypes, params GenericFuncContainer<T,bool>[] comparers)
			where T : IMemberDefinition
		{
			if (type != null)
			{
				...
				if (searchInNestedTypes)
				{
					foreach (TypeDefinition nestedType in tdef.NestedTypes)
						members.AddRange(findMembers<T>(module, nestedType, true, comparers));
				}
			...
			else
			{
				...
				{
					members.AddRange(findMembers<T>(module, tdef, searchInNestedTypes, comparers));
				}
				return members.ToArray();
			}
		}
		...
		public static void executeActions<T>(ModuleDefinition module, object type, GenericFuncContainer<T,bool>[] comparers, params Func<T,bool>[] actions)
			where T : IMemberDefinition
		{
			T[] members = findMembers<T>(module, type, false, comparers);
			if (members.Length == 0)
				OnError (ErrorCode.MEMBER_NOT_FOUND, typeof(T).Name, Environment.StackTrace);
		...
		public static bool RenameVirtualMethod(MethodDefinition method, string newName)
		{
			...
			foreach (MethodDefinition curVMethod in findMembers<MethodDefinition>(curBaseType.Module, null, false, vMethodComparer))
			{
